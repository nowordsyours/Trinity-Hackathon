...
toilets_by_cleaner = {}  # Index toilets by cleaner_assigned
toilets_by_id = {}       # Index toilets by ID

def rebuild_toilet_indexes():
    """Rebuild optimized toilet indexes"""
    global toilets_by_cleaner, toilets_by_id
    toilets_by_cleaner = {}
    toilets_by_id = {}
    ...
    for toilet in toilets_data:
        ...
        # Index by ID
        toilets_by_id[toilet_id] = toilet
        
        # Index by cleaner
        if cleaner_assigned not in toilets_by_cleaner:
            toilets_by_cleaner[cleaner_assigned] = []
        toilets_by_cleaner[cleaner_assigned].append(toilet)

# Build initial indexes
rebuild_toilet_indexes()
...
def simulate_real_time_updates():
    ...
    global real_time_updates, toilets_data  # Access the global variables
    while True:
        time.sleep(10)  # Update every 10 seconds as requested
        
        # Update hygiene scores for all toilets with realistic depreciation
        for toilet in toilets_data:
            # Only depreciate hygiene score if it's above minimum threshold
            if toilet['hygiene_score'] > 10:
                # Depreciate by 5-10% every 10 seconds as requested
                depreciation_percent = random.uniform(0.05, 0.10)  # 5-10%
                score_drop = toilet['hygiene_score'] * depreciation_percent
                new_score = max(10, toilet['hygiene_score'] - int(score_drop))
                
                # Only create update if score actually changed
                if new_score != toilet['hygiene_score']:
                    old_score = toilet['hygiene_score']
                    toilet['hygiene_score'] = new_score
                    toilet['last_updated'] = datetime.now()
                    
                    # Update status based on new hygiene score
                    ...
                    # Create update notification for hygiene score change
                    update = {
                        'id': str(uuid.uuid4()),
                        'toilet_id': toilet['id'],
                        'toilet_name': toilet['name'],
                        'previous_status': old_status,
                        'new_status': toilet['status'],
                        'old_score': old_score,
                        'new_score': new_score,
                        'timestamp': datetime.now(),
                        'type': 'hygiene_depreciation',
                        'icon': toilet['icon']
                    }
                    real_time_updates.append(update)
        
        # Also randomly update one toilet condition for variety
...
@app.route('/')
def index():
    """Main landing page - redirect to fresh auth design"""
...
@app.route('/auth')
def auth():
    """Authentication page with fresh design"""
    return render_template('auth_fresh.html')
...
def get_priority_toilets():
    ...
    try:
        ...
        cleaner_toilets = toilets_by_cleaner.get(user_email, [])
        
        # Calculate priority based on hygiene score and time since last cleaning
        priority_toilets = []
        for toilet in cleaner_toilets:
...
def get_staff_stats():
    ...
    try:
        ...
        rating_count = 0
        on_time_count = 0
        total_tasks = len(cleaner_toilets)
...
def start_cleaning():
    ...
    try:
        ...
        # Rebuild indexes after toilet data changes
        rebuild_toilet_indexes()
...
def get_staff_updates():
    ...
    try:
        ...
        # Get recent cleaning updates (last 15 minutes)
        recent_cleaning_updates = []
        current_time = datetime.now()
        for update in real_time_updates[-15:]:  # Last 15 updates
            if isinstance(update['timestamp'], datetime):
                time_diff = (current_time - update['timestamp']).total_seconds() / 60  # minutes
                if time_diff <= 15:  # Only include updates from last 15 minutes
                    recent_cleaning_updates.append({
                        'id': update['id'],
        ...
        has_updates = len(new_urgent_toilets) > 0 or len(recent_cleaning_updates) > 0
        has_cleaning_updates = len(recent_cleaning_updates) > 0
        
        return jsonify({
            'has_updates': has_updates,
            'has_cleaning_updates': has_cleaning_updates,
            'new_urgent_toilets': new_urgent_toilets,
            'recent_cleaning_updates': recent_cleaning_updates,
            'total_updates': len(real_time_updates),
...
def get_toilet_directions(from_toilet_id, to_toilet_id):
    ...
    try:
        from_toilet = toilets_by_id.get(from_toilet_id)
        to_toilet = toilets_by_id.get(to_toilet_id)
