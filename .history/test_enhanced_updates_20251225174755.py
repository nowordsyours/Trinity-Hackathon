...
def test_get_priority_toilets(session):
    """Test getting priority toilets for staff"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Found {len(urgent_toilets)} urgent toilets")
            
            # Show first few toilets
            for i, toilet in enumerate(urgent_toilets[:3]):
                print(f"   {i+1}. {toilet['name']} - Score: {toilet['hygiene_score']} - Status: {toilet['status']}")
            
            return urgent_toilets
        else:
